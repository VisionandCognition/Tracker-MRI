...
Stm.KeepFixT = [800 300]; % [mean sd] of NORM % time to fix before target onset. not <|> m+/-sd
...
Stm.HoldTarT = 250; % stay in target window for this long
...
Stm.RewardDelay = .5*[1000 2500]; % delay before reward is given [min max]
...
elseif TarCreateAlgorithm == 3
    ...
    for c=1:length(Stm.PolarAngles)
        ...
        %Stm.Cond(c).Targ(1).PreTargCol = 3*Stm.Cond(c).Targ(1).Color;
        Stm.Cond(c).Targ(1).PreTargCol = [0.65 0.65 0.65];
        Stm.Cond(c).Targ(1).Reward = 0.140;
        Stm.Cond(c).Targ(1).RewardGain = 1;
        Stm.Cond(c).Targ(1).RewDelayOutlineWidth = 0.05;
        Stm.Cond(c).Targ(2) = Stm.Cond(c).Targ(1);
        Stm.Cond(c).Targ(2).Position = -Stm.Cond(c).Targ(2).Position;
    end
end
