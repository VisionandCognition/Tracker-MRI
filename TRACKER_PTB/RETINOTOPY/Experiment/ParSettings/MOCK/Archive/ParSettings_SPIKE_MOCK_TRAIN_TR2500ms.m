...
% if 2 positions are defined its [target1; target2]
% Par.RespIndPos = [-0 0; 0 0]; % deg

% if multiple (>2) positions are defined the position will be chosen
% randomly from the defined options
r=3; rr=sqrt((r^2)/2);
Par.RespIndPos = [...
     0  0;...
    -r  0;...
...
Par.TaskReward_HandInGain = [1 1.1]; % one hand , both hands
