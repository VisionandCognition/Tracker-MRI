...
if Stm.nRepeatsStimSet == 0
    ...
    NumVolNeeded = Stm.PreDur_TRs + ...
        Stm.firstint_TRs + ...
        Stm.nRepeatsStimSet*(...
        length(Stm.FigGnd)*(...
        Stm.stim_rep*(Stm.stim_TRs+Stm.int_TRs)))+...
        Stm.PostDur_TRs;
...
while ~Par.ESC && ~ExpFinished
    ...
    if ~Par.ToggleHideStim && ~Par.HideStim_BasedOnHandIn(Par) && ~Par.Pause
        ...
        if strcmp(ExpStatus,'PreDur') || strcmp(ExpStatus,'PostDur')
            ...
            elseif strcmp(Stm.StimType{2},'dots_tex') && ...
                    (~strcmp(CurrentlyDrawn,'PreDur') && FlipScreenNow)
                if Stm.FigGnd{Log.StimOrder(1)}(1,2) ~= 0    
                    Screen('FillRect',Par.window,Stm.Gnd_all.backcol*Par.ScrWhite)
                    % dot texture
...
%                         [],Stm.Gnd_all.dots.type);
        ...
        elseif strcmp(ExpStatus,'StimBlock') && strcmp(WithinBlockStatus,'Stim')          
            ...
            elseif strcmp(Stm.StimType{2},'dots') && FlipScreenNow
                ...
                if Stm.MoveStim.Do && StimStarted && ...
                    ...
                    for d=1:2
                        if Stm.FigGnd{Log.StimOrder(StimNr)}(sidx,2)
                ...
                if strcmp(StimType,'Figure') && ...
                    ...
                    if ~Stm.FigGnd{Log.StimOrder(StimNr)}(sidx,2) % no background
                        Screen('FillRect',Par.window,Stm.Gnd_all.backcol*Par.ScrWhite,...
                            stimulus.Fig(Stm.FigGnd{Log.StimOrder(StimNr)}(sidx,1)).RectDest)
                    end
                    Screen('DrawDots', Par.window, ...
    ...
    if FlipScreenNow
        ...
        lft=GetSecs;
        if lft-prevlft > 0.010
            % update fixation performance logging
            UpdateFixLog = true;
        end
    end
    ...
    switch ExpStatus
        case 'PreDur'
            ...
            if lft-Log.StartPre >= (Stm.PreDur_TRs*Par.TR) - DrawDur
                ExpStatus = 'StimBlock';
                if Stm.int_TRs > 0
        ...
        case 'StimBlock'
            switch WithinBlockStatus
                case 'FirstInt'
                    ...
                    % check for end of period ---
                    if lft-Log.StartInt >= (Stm.firstint_TRs*Par.TR) - DrawDur
                        WithinBlockStatus = 'Stim';
                        StimLogDone = false;
                        StimNr = 1;
                        StimRepNr = 1;
                ...
                case 'Stim'
                    switch StimType
                        case 'Figure'
                            % end of figure
                            if lft-Log.StartStim >= (Stm.stim_TRs*Par.TR) - DrawDur
                                Log.nEvents = Log.nEvents+1;
                                LogCollect = [LogCollect; {Log.nEvents,[],'FigGnd',...
                                    'Figure','stop'}];
                                LastStim = 'Figure';
...
%                             % don't change srcrect anymore
                ...
                case 'Int'
                    if lft-Log.StartInt >= (Stm.int_TRs*Par.TR) - DrawDur
                        Log.nEvents = Log.nEvents+1;
                        LogCollect = [LogCollect; {Log.nEvents,[],'FigGnd',...
                            'Intermediate','stop'}];
