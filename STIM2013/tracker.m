...
[FileName,PathName] = uigetfile('PAR.mat','Select a parameter Mat-file');
...
%     TargSz = Par.TargSzDeg * Par.PixPerDeg;
        for i = 1:size(Par.WIN,2)
            ...
            elseif Par.WIN(5,i) == 1 || Par.WIN(5,i) == 2 %target window == 2
                ...
                Par.WIN(4,i) = Par.TargHtDeg * Par.PixPerDeg; %height
            end
        end
...
% --------------------------------------------------------------------
function MI_Run_Callback(hObject, eventdata, handles)
% hObject    handle to MI_Run (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    global Par
    FSTR = uigetfile('*.m');
